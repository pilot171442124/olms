...
$sheetName = "Data";
$reportHeaders = array('Online Library Management System','Book Request List');

$writer->writeSheetHeader(
			$sheetName, 
			$header, 
			array(
				'widths'=>array(8,20,15,25,25,20),
				'font-style'=>'bold',
				'font-size'=>11,
				'fill'=>'#b4c6e7',
...
$rowStyle = array( 'border'=>'left,right,top,bottom','border-style'=>'thin');
 ...
 $sql = "SELECT a.*,BookName,Department
...
while ($val = mysqli_fetch_array($result)) {
	...
	else if($val['Status'] == 'Issued'){
		$Status = "Received";
	}
	else if($val['Status'] == 'Dateover'){
		$Status = "Date Over";
	}
	...
	$writer->writeSheetRow($sheetName, array($i++, $val['RequestCode'],$val["RequestDate"],$val['Department'],$val['BookName'],$Status),$rowStyle );

}

//Report header merge
if(count($reportHeaders)>0){
	$writer->markMergedCell($sheetName, $start_row=0, $start_col=0, $end_row=0, $end_col=5);
	$writer->markMergedCell($sheetName, $start_row=1, $start_col=0, $end_row=1, $end_col=5);
} 


$exportFileName="book_request_list_".date("Y_m_d_H_i_s").".xlsx";
